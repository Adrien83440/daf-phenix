...
**Nova, Phénix en direct** (Premium) : une assistante vocale plein écran, nommée Nova (`PERSO_VOCAL_NOM` côté serveur, `assistant` dans `window.PERSO_CONFIG`). Un gros bouton « Parler à Nova » en tête de l'espace, un bouton flottant sur les autres écrans ; à l'ouverture elle dit « Je t'écoute, dis-moi » et écoute aussitôt ; une nappe sonore douce pendant la réflexion et des blips à l'écoute et à la réponse (générés dans le navigateur, coupables d'un clic). Un orbe animé sur canvas réagit au niveau réel du micro à l'écoute, respire pendant la réflexion et se module pendant la réponse. La reconnaissance vocale est celle du navigateur (Chrome, Safari, iPhone ; repli en mode écrit sinon). La voix, elle, est **neuronale** dès qu'une clé est posée : `ELEVENLABS_API_KEY` (recommandé, le plus naturel en français ; `ELEVENLABS_VOICE_ID` pour choisir la voix dans la bibliothèque, sinon une voix française du compte est prise) ou `OPENAI_API_KEY` (`gpt-4o-mini-tts`, voix `OPENAI_TTS_VOICE`, `nova` par défaut). L'audio est servi par la fonction et analysé dans le navigateur : l'orbe se module sur la vraie voix. Sans clé, voix du navigateur. La conversation tourne sur un modèle rapide (`PERSO_VOCAL_MODEL`, Haiku 4.5 par défaut) pour répondre en une à deux secondes. La personne raconte ce qui se passe ; l'IA répond en deux à quatre phrases à voix haute, et si le changement est durable (travail, logement, dette, projet) elle réécrit le paragraphe de situation, l'annonce et laisse annuler ; elle peut proposer une action à ajouter à la semaine. Chaque tour est un appel court (effort bas, 1 200 jetons), compté à part des bilans (`PERSO_VOCAL_QUOTA`, 40 par jour). L'accès se donne depuis la console (case « Premium » sur la fiche ou à la création) ou pour tous avec `PERSO_VOCAL_POUR_TOUS=1`.
