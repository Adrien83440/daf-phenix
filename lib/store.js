...
//   - "kv"     : Vercel KV / Upstash Redis via son API REST (aucune dépendance
//                npm). Activé dès que KV_REST_API_URL et KV_REST_API_TOKEN
...
//  Moteur mémoire
// ---------------------------------------------------------------------------
const mem = { clients: new Map(), revoked: new Set(), runs: new Map(), days: new Map(), clientStats: new Map(), events: [], admins: new Map() };
...
function _reset() { mem.clients.clear(); mem.revoked.clear(); mem.runs.clear(); mem.days.clear(); mem.clientStats.clear(); mem.events.length = 0; mem.admins.clear(); }

module.exports = { backend, today, dayList, saveClient, getClient, listClients, deleteClient, setRevoked, isRevoked, revokedList, consumeRun, quotaUsed, record, traced, events, dayStats, clientStats, getAdmins, setAdmin, deleteAdmin, ping, STAT_FIELDS, _reset };
